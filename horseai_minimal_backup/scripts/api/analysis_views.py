"""
ПОЛНЫЙ API для работы с анализами
"""
...
def user_analyses(request):
    """Список анализов текущего пользователя"""
    if not HAS_MODELS:
        return Response({
            'success': False,
    ...
    try:
        ...
        analyses = Analysis.objects.filter(video__in=videos).order_by('-analysis_date')
        
        # Формируем ответ
        data = []
        for analysis in analyses:
            video = analysis.video
            animal = video.animal if video else None
            ...
            data.append({
                ...
                'video_filename': os.path.basename(video.file_path) if video and video.file_path else 'Неизвестно',
                ...
                'estimated_weight': float(analysis.estimated_weight) if analysis.estimated_weight else None,
            })
