"""
ПРОСТОЙ И РАБОЧИЙ API для животных
Ничего не удаляет, только добавляет новые endpoints
"""
...
def animal_stats(request, animal_id):
    ...
    try:
        ...
        video_count = Video.objects.filter(animal=animal).count()
        analysis_count = Analysis.objects.filter(video__animal=animal).count()
        ration_count = Ration.objects.filter(animal=animal).count()
        ...
        return Response({
            ...
            'stats': {
                ...
                'last_analysis_date': last_analysis.analysis_date if last_analysis else None,
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def bulk_animals_check(request):
    """Массовая проверка доступности животных (для фронтенда)"""
    try:
        custom_user = User.objects.get(login=request.user.username)
        ...
        return Response({
            ...
            'total_available': len(available_animals)
        })
