"""
ИСПРАВЛЕННЫЙ ML Views с правильными импортами
"""
import json
...
# ИСПРАВЛЕННЫЙ ИМПОРТ - используем только существующие модели
try:
    from web.database.models import Animal, Video, Analysis
    print("✅ Импорт моделей успешен: Animal, Video, Analysis")
except ImportError as e:
    print(f"⚠️ Ошибка импорта моделей: {e}")
    # Пытаемся импортировать по отдельности
...
def run_your_detector(video_path, video_id):
    ...
    """
    try:
    print("DEBUG: Начало video_upload_page")
...
# В файле /home/ais/shared/horseAI/frontend/ml_views.py
# Найдите функцию video_upload_page и исправьте:


@login_required
def video_upload_page(request):
    """Страница загрузки видео"""
    print("=== VIDEO UPLOAD PAGE ===")
    ...
    try:
        ...
        if isinstance(request.user, str):
            user_obj = User.objects.get(login=request.user)
        else:
            user_obj = request.user

        print(f"Пользователь найден: {user_obj.login}")

        # Получаем животных пользователя
        animals = Animal.objects.filter(user=user_obj)
        print(f"Найдено животных: {animals.count()}")

        return render(request, 'frontend/video_upload.html', {'animals': animals})

    except Exception as e:
        print(f"Ошибка: {e}")
        # Временно показываем всех животных
        ...
        print(f"Пользователь '{request.user}' не найден в БД")
        ...
        print(f"Ошибка в video_upload_page: {e}")
        return render(request, 'frontend/video_upload.html', {'animals': []})
